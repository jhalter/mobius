...
build-linux-arm-server:
	...
	GOOS=darwin GOARCH=arm go build -o $(linux_server_arm_target)/mobius-hotline-server  cmd/mobius-hotline-server/main.go
...
build-linux-amd64-server:
	...
	GOOS=darwin GOARCH=amd64 go build -o $(linux_server_amd64_target)/mobius-hotline-server  cmd/mobius-hotline-server/main.go
